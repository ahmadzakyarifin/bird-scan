  ...
  <div class="w-full max-w-4xl mx-auto pointer-events-auto bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-2xl p-8 rounded-3xl shadow-2xl border border-yellow-500/20 transform transition-all duration-500 translate-y-0 opacity-100 flex flex-col h-[80vh]">
    ...
    <div class="flex justify-between items-start mb-6 shrink-0">
      <div>
        ...
        <h2 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Botanical Garden</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Tanaman yang berhasil Anda temukan dan dokumentasikan akan tumbuh di kebun ini.</p>
      </div>
    ...
    <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar">
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <!-- Collected Plant Cards -->
        <div v-for="plant in collectedPlants" :key="plant.id" class="bg-gray-50 dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 hover:border-emerald-500/50 transition-colors cursor-pointer group flex flex-col items-center text-center">
          <div class="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center text-4xl mb-3 shadow-inner group-hover:scale-110 transition-transform">
            {{ plant.icon }}
          </div>
          <h3 class="font-bold text-gray-900 dark:text-white text-sm">{{ plant.name }}</h3>
          <span class="text-[10px] text-emerald-600 dark:text-emerald-400 font-mono mt-1">{{ plant.family }}</span>
          <div class="mt-3 text-xs text-gray-500 dark:text-gray-400 bg-gray-200 dark:bg-gray-800 px-2 py-1 rounded w-full">
            Ditemukan: {{ plant.date }}
          </div>
        </div>
    ...
    <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-800 flex justify-between items-center shrink-0">
      <div class="flex items-center gap-3">
        <div class="w-12 h-12 rounded-full bg-emerald-500 flex items-center justify-center text-white font-bold text-lg shadow-lg">
          {{ collectedPlants.length }}
        </div>
        <div>
          <h4 class="font-bold text-gray-900 dark:text-white text-sm">Level Peneliti: Pemula</h4>
          <div class="w-32 h-1.5 bg-gray-200 dark:bg-gray-800 rounded-full mt-1 overflow-hidden">
            <div class="h-full bg-emerald-500 rounded-full" :style="{ width: (collectedPlants.length / (collectedPlants.length + emptySlots)) * 100 + '%' }"></div>
          </div>
        </div>
      </div>
...
// Mock Data
const collectedPlants = ref([
  { id: 1, name: 'Monstera Deliciosa', family: 'Araceae', icon: '🌿', date: 'Hari ini' },
  { id: 2, name: 'Rafflesia Arnoldii', family: 'Rafflesiaceae', icon: '🌺', date: 'Kemarin' },
  { id: 3, name: 'Nepenthes (Kantong Semar)', family: 'Nepenthaceae', icon: '🥀', date: '2 hari lalu' },
])

const emptySlots = 9 // Menunjukkan slot yang belum ditemukan
</script>
